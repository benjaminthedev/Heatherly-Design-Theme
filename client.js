jQuery(function ($) {
  ...
  $(document).ready(function () {
    ...
    //Filter for Fabrics Sample Page
    $("#fabrics-search").on("keyup", function () {
      var value = $(this).val().toLowerCase();
      $(".single-sample-swatch").filter(function () {
        $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
      });
    });

    //Filter for Bed Product Page


    /////USE A JS TIME FUNC DELAY THE LOAD?????
    var myVar = setTimeout(myTimer, 4000);

    function myTimer() {
      console.log('Loaded Now!! 4 seconds New Swatch Search Timers!)');
      console.log('working');
      $("#fabrics-search-single").on("keyup", function () {
        var valueSingle = $(this).val().toLowerCase();
        console.log(valueSingle);
        $(".single-swatch").filter(function () {
          $(this).toggle($(this).text().toLowerCase().indexOf(valueSingle) > -1)
        });
      });
    };
